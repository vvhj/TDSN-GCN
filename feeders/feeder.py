...
class Feeder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     data_numpy = random_choose_simple(data_numpy, self.window_size, center=True)
        # else:
        #     data_numpy = random_choose_simple(data_numpy, self.window_size)
        if self.bone:
            from .bone_pairs import openpose_pairs
            bone_data_numpy = np.zeros_like(data_numpy)
            for v1, v2 in openpose_pairs:
                bone_data_numpy[:, :, v1 ] = data_numpy[:, :, v1 ] - data_numpy[:, :, v2 ]
                #bone_data_numpy[2, :, v1 ] = (data_numpy[2, :, v1 ] + data_numpy[2, :, v2 ])/2
