...
def valid_crop_resize(data_numpy,valid_frame_num,p_interval,window,vel=None,cropped_length=64):
    # input: C,T,V,M
    ...
    if len(p_interval) == 1:
        ...
        cropped_length = np.minimum(np.maximum(int(np.floor(valid_size*p)),cropped_length), valid_size)# constraint cropped_length lower bound as 64
